...
CCSequence* AnimatePacker::getSequence(const char *name){
	...
	if(animation)
	{
		...
		actions->addObject(CCFlipX::actionWithFlipX(animate.flipX));
		actions->addObject(CCFlipY::actionWithFlipY(animate.flipY));
		actions->addObject(CCAnimate::actionWithAnimation(animation));
