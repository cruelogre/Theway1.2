...
--------------------------------------------------------------------------
local Personal = class("Personal",require("WhippedEgg.logics.RoomBase"))
...
function Personal:ctor()
	Personal.super.ctor(self)
end

function Personal:init()
	self.logTag = "Personal.lua"
	--这里要判断一下信息是否已经进来了 最后一个人进来的时候 数据最先过来
	local Personalstart = DataCenter:getData(WhippedEggCfg.InnerEvents.GD_EVENT_GAMESTART)
	if Personalstart and next(Personalstart) then
		local event  = {name = WhippedEggCfg.InnerEvents.GD_EVENT_GAMESTART }
		wwlog(self.logTag, "进来前就收到私人房开局消息")
		self:commondEventHandle(event)
	else
	 	Personalstart = DataCenter:getData(WhippedEggCfg.InnerEvents.GD_EVENT_RESUMEGAME)
	 	if Personalstart and next(Personalstart) then
	 		local event  = {name = WhippedEggCfg.InnerEvents.GD_EVENT_RESUMEGAME }
			wwlog(self.logTag, "进来前就收到私人房恢复对局消息")
			self:commondEventHandle(event)
	 	end
	end
	...
	local _,handler8 = WhippedEggCfg.innerEventComponent:addEventListener(WhippedEggCfg.InnerEvents.GD_EVENT_MATCH_HOLDAWARD,handler(self,self.commondEventHandle))
	local _,handler9 = WhippedEggCfg.innerEventComponent:addEventListener(WhippedEggCfg.InnerEvents.GD_EVENT_USERINFO_RESP,handler(self,self.commondEventHandle))
	...
	self.sirenHandlers = {}
    local listener1 = WWFacade:addCustomEventListener(SiRenRoomCfg.InnerEvents.SIREN_ROOM_BALANCE,handler(self,self.sirenCallFuc))
	local listener2 = WWFacade:addCustomEventListener(SiRenRoomCfg.InnerEvents.SIREN_ROOM_NOTIFY,handler(self,self.sirenCallFuc))

	
	table.insert(self.sirenHandlers,listener1)
	table.insert(self.sirenHandlers,listener2)


	--开局之前有玩家离开
	self.leaveBeforeStartGamePlayId = {} 
end

function Personal:sirenCallFuc( event )
	-- body
	if event._eventName == SiRenRoomCfg.InnerEvents.SIREN_ROOM_BALANCE then --私人房结束
		local WhippedEggSceneController = ControllerMgr:retrieveController(ControllerMgr:getControllerRegistry().WHIPPEDEGG_SCENE)
		wwlog("当前私人房结算房间号","%d   %d",WhippedEggSceneController.gameZoneId,event._userdata[1].RoomID)
		if WhippedEggSceneController.gameZoneId == event._userdata[1].RoomID then
			self:handlePersonalNewGame(event._userdata[1].result)
		end
	elseif event._eventName == SiRenRoomCfg.InnerEvents.SIREN_ROOM_NOTIFY then
		local data = event._userdata
		if data[1].Type == 7 then --离开
			wwlog("有玩家离开")
			if self.StartGame then
				local playidPosition = GameManageFactory:getCurGameManage():getPositionbyId(self.seatInfos, data[1].Param1)
				GameManageFactory:getCurGameManage():playerPersonalLeave(playidPosition)
			else
				table.insert(self.leaveBeforeStartGamePlayId,data[1].Param1)
			end
		elseif data[1].Type == 8 then --返回
			wwlog("有玩家返回")
			for i=#self.leaveBeforeStartGamePlayId,1,-1 do
				if self.leaveBeforeStartGamePlayId[i] == data[1].Param1 then
					table.remove(self.leaveBeforeStartGamePlayId,i)
				end
			end
			local playidPosition = GameManageFactory:getCurGameManage():getPositionbyId(self.seatInfos, data[1].Param1)
			GameManageFactory:getCurGameManage():playerPersonalComeBack(playidPosition)
		elseif data[1].Type == 2 then --解散
	  		Toast:makeToast(i18n:get('str_guandan','guandan_SirenJieSan'), 1.0):show(function ( ... )
	  			-- body
...
--重新滞空
function Personal:recycle()
	Personal.super.recycle(self)
	
	for k,v in ipairs(self.sirenHandlers) do
...
function Personal:createCardRecorder( ... )
	Personal.super.createCardRecorder(self)
...
--]]
function Personal:handleStartGame(eventid)
	wwlog(self.logTag, "私人房开局处理")
	if self.StartGame then
		return
	end
	self.StartGame = true
    ...
    Personal.super.baseStartGame(self,self.startData)
end

function Personal:handleFirstStepEnd( ... )
	-- body
	Personal.super.handleFirstStepEnd(self)
	--开局没进来的玩家显示离开
	for k,v in pairs(self.leaveBeforeStartGamePlayId) do
		local playidPosition = GameManageFactory:getCurGameManage():getPositionbyId(self.seatInfos,v)
		GameManageFactory:getCurGameManage():playerPersonalLeave(playidPosition)
	end
end

--续局回调
function Personal:HandleContinueStep()
...
--玩家进贡选牌回调
function Personal:handleTributeChoose( choosePoker, type )
...
--收到进贡回调消息了哦
function Personal:handleTribute(eventid)
...
--收到交换牌消息了哦
function Personal:handleExchangeCard(eventid)
	if not self.StartGame then
		return
	end
...
--结算
function Personal:handleGameOver(eventid)
	if not self.StartGame then
		return
	end
	...
	Personal.super.handleGameOver(self,eventid)
end	

function Personal:handleResumeGame(eventid)
	--恢复对局	
	wwlog(self.logTag, "私人恢复对局处理")
	Personal.super.resumeGameSet(self)
	...
	if GameManageFactory.gameType == Game_Type.PersonalPromotion or 
		...
		for k,v in pairs(sirenData.userInfo) do
			if v.UserID ~= self.seatInfos.side1[1].UserID then
				local PositionType = GameManageFactory:getCurGameManage():getPositionbyId(self.seatInfos,v.UserID)
				if v.Status == 2 then
					GameManageFactory:getCurGameManage():playerPersonalLeave(PositionType)
				elseif v.Status == 1 then
					GameManageFactory:getCurGameManage():playerPersonalComeBack(PositionType)
				end
			end
		end
		if GameManageFactory:getCurGameManage().BackGrand then
			GameManageFactory:getCurGameManage().BackGrand:resetDouble(self.startData.RoomMultiple)
		end
	end
    Personal.super.baseResumeGameGame(self,self.startData)
end

--桌子状态改变
function Personal:handleTableState(eventid)
...
function Personal:handleHoldAward(eventid)
...
function Personal:handleUserInfo(eventid,reqUserid)
	if not self.StartGame then
		return
	end
...
function Personal:handlePersonalNewGame(info)
	wwlog(self.logTag,"私人房结束了 重新开房")
	GameManageFactory:getCurGameManage():PersonalEnd(info)
end
