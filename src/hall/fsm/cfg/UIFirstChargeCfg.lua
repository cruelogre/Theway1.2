...
--------------------------------------------------------------------------
local registry = {
    stateName = "UIFirstChargeState";-- 状态机模块名（见名知意即可）
    resData = { };-- 状态机对象所用到的所有资源
    controller = "hall.fsm.stateModelControl.UIFirstChargeState";-- 控制器 配置UISettingState.lua
    view = "hall.mediator.view.FirstChargeLayer"; --加载完后创建的界面,可不配置（如不配置请在UISettingState里面去自动调用视图构建）
...
{
	"hall/firstCharge/firstCharge_plist.png",
    "hall/store/store_plist.png",
}
registry.resData.Plist = { 
	"hall/firstCharge/firstCharge_plist.plist",
