...
--------------------------------------------------------------------------
local registry = {
    stateName = "UIExchangeState";-- 状态机模块名（见名知意即可）
    resData = { };-- 状态机对象所用到的所有资源
    controller = "hall.fsm.stateModelControl.UIExchangeState";-- 控制器 配置UISettingState.lua
    view = "hall.mediator.view.ExchangeLayer";-- 加载完后创建的界面,可不配置（如不配置请在UISettingState里面去自动调用视图构建）
...
{
--    "hall/exchange/exchange.png",
}
registry.resData.Plist =
{
    "hall/exchange/exchange.plist",
}
