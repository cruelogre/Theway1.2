...
local MatchLayer_widget_Face2Face2 = class("MatchLayer_widget_Face2Face2",ccui.Layout)
...
function MatchLayer_widget_Face2Face2:ctor(size,numbers)
	self.size = size
	self.numbers = numbers or MatchCfg.searchSendnumbers
	self.timeCount = 0
	...
	self:setName("MatchLayer_widget_Face2Face2")
	self:init()
	
end

function MatchLayer_widget_Face2Face2:init()
	print("MatchLayer_widget_Face2Face2:init")
	...
	self.timeCount = os.time() - MatchCfg.matchSendnumberTime
	print("self.timeCount",self.timeCount)
	self.timeCount = math.max(self.timeCount,0)
	self:freshLeftTime(MatchCfg.numberValidTime - self.timeCount)
	--self.clockNumber:setString("0")
	self:freshTextNumber(self.numbers)
	self:freshButton()
end 

function MatchLayer_widget_Face2Face2:freshTextNumber(numbers)
	local showStr = ""
...
function MatchLayer_widget_Face2Face2:freshButton()
...
--刷新内容
function MatchLayer_widget_Face2Face2:freshContent(event)
	
	local foundmates = DataCenter:getData(MatchCfg.InnerEvents.MATCH_EVENT_FOUNDMATES_FACE)
	
	if not foundmates or not next(foundmates) then
		print("MatchLayer_widget_Face2Face2 没找到对象")
		return
	end
	...
	print("MatchLayer_widget_Face2Face2:freshContent....")
	
	--有好友啦
	--add custom item
	self:showFriends(foundmates)
	
end
function MatchLayer_widget_Face2Face2:showFriends(friendMates)
...
function MatchLayer_widget_Face2Face2:eventComponent()
	return MatchCfg.innerEventComponent
end

function MatchLayer_widget_Face2Face2:bindTimeOutCB(cb)
	self.timoutCB = cb
end

function MatchLayer_widget_Face2Face2:bindChangeFun(cb)
	self.changeCB = cb
end

function MatchLayer_widget_Face2Face2:onEnter()
--[[	self:runAction(cc.Sequence:create(cc.DelayTime:create(2.0),cc.CallFunc:create(function ()
	...
	if self:eventComponent() then
		...
		MatchCfg.InnerEvents.MATCH_EVENT_FOUNDMATES_FACE,handler(self,self.freshContent))
	...
	self.friendListView:removeAllItems()
	local foundmates = DataCenter:getData(MatchCfg.InnerEvents.MATCH_EVENT_FOUNDMATES_FACE_ALL)
	if foundmates and foundmates.mateList and next(foundmates.mateList) then
		self.radarImg:setPositionY(self.orPosY - 100)
		self.radarImg:setScale(0.8)
		local tempMateList = {}
		copyTable(foundmates.mateList,tempMateList)
...
function MatchLayer_widget_Face2Face2:touchListener(ref,eventType)
	if not ref then
		return
	end
...
function MatchLayer_widget_Face2Face2:timeClick()
	self.timeCount = self.timeCount + 1
	--print("timeClick",self.timeCount)
	self:freshLeftTime(MatchCfg.numberValidTime - self.timeCount)
	if self.timeCount >= MatchCfg.numberValidTime then --时间到了，停止记时 
...
function MatchLayer_widget_Face2Face2:freshLeftTime(t)
	t = math.max(t,0)
	self.clockNumber:setString(tostring(t))
end

function MatchLayer_widget_Face2Face2:onExit()
	stopSoundEffect("sound/effect/leida")
...
function MatchLayer_widget_Face2Face2:active()
	print("MatchLayer_widget_Face2Face2 active")
	self.timeCount = 0
end

return MatchLayer_widget_Face2Face2