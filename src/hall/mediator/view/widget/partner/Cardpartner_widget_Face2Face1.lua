...
-- Date:    2017.1.23
-- Last: 
-- Content:  牌友界面添加好友控件 面板
-- Copyright (c) wawagame Entertainment All right reserved.
--------------------------------------------------------------------------

local Cardpartner_widget_Face2Face1 = class("Cardpartner_widget_Face2Face1",ccui.Layout)
...
function Cardpartner_widget_Face2Face1:ctor(size)
	self.size = size

	self:setContentSize(size)
	self:setAnchorPoint(cc.p(0.5,0.5))
	self:setTouchEnabled(true)
	self:setName("Cardpartner_widget_Face2Face1")
	self:init()
	
end

function Cardpartner_widget_Face2Face1:init()
	print("Cardpartner_widget_Face2Face1:init")
...
--数字输入完毕的回调
function Cardpartner_widget_Face2Face1:bindChangeFun(cbFun)
	
	self._cbFun = cbFun
end

function Cardpartner_widget_Face2Face1:eventComponent()
	
end

function Cardpartner_widget_Face2Face1:onEnter()
...
function Cardpartner_widget_Face2Face1:addNumber(number)
	if self.inputIndex<= #self.numbers then
		...
		if self.inputIndex>#self.numbers then
			...
			if self._cbFun then
					--清空搜索到的好友信息
					DataCenter:clearData(CardPartnerCfg.InnerEvents.CP_EVENT_FOUNDMATES_FACE)
					DataCenter:clearData(CardPartnerCfg.InnerEvents.CP_EVENT_FOUNDMATES_FACE_ALL)
					--请求服务器
					--MatchProxy:requestFriend(3,table2Number(self.numbers,4))
					SocialContactProxy:requestFaceBuddy(table2Number(self.numbers,4))
					CardPartnerCfg.matchSendnumberTime = os.time() --当前发送数字的时间
					CardPartnerCfg.searchSendnumbers = table2Number2(self.numbers,4)
					self._cbFun(CardPartnerCfg.searchSendnumbers)	
...
function Cardpartner_widget_Face2Face1:delete()
	self.inputIndex = self.inputIndex - 1
...
function Cardpartner_widget_Face2Face1:clear()
	for x=1,#self.numbers do
		self:delete()
	end
end

function Cardpartner_widget_Face2Face1:touchListener(ref,eventType)
	if not ref then
		return
	end
...
function Cardpartner_widget_Face2Face1:onExit()
	
	
end


return Cardpartner_widget_Face2Face1